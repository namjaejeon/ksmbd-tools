...
static struct cifsd_dcerpc *dcerpc_alloc(unsigned int flags, int sz)
{
	...
	dce = calloc(1, sizeof(struct cifsd_dcerpc));
	if (!dce)
		return NULL;
...
void dcerpc_set_ext_payload(struct cifsd_dcerpc *dce, void *payload, size_t sz)
{
	dce->num_pointers = 1;
	dce->payload = payload;
	dce->payload_sz = sz;
...
static struct cifsd_rpc_pipe *rpc_pipe_alloc(void)
{
	...
	pipe = calloc(1, sizeof(struct cifsd_rpc_pipe));
	if (!pipe)
		return NULL;
...
int ndr_write_##name(struct cifsd_dcerpc *dce, type value)		\
{									\
	type ret;							\
									\
...
type ndr_read_##name(struct cifsd_dcerpc *dce)				\
{									\
	type ret;							\
									\
	align_offset(dce, sizeof(type));				\
...
int ndr_write_union_##name(struct cifsd_dcerpc *dce, type value)	\
{									\
	int ret;							\
									\
		     ...
		     struct cifsd_rpc_command *resp)
{
	...
	pipe->dce = dcerpc_ext_alloc(CIFSD_DCERPC_LITTLE_ENDIAN |
				     CIFSD_DCERPC_ALIGN4,
				     req->payload,
				     req->payload_sz);
	if (!pipe->dce) {
